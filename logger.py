...
def log_model(model):
    ...
    # writer.add_histogram("state.memory.data", to_log(model.DNCmemory.memory), global_step, bins='sturges')
    # writer.add_histogram("state.memory", to_log(model.DNCmemory.memory), global_step, bins='sturges')
...
def log_state(state):
    # print(global_step)
    if global_step % log_step == 0 and len(state) == 8:
