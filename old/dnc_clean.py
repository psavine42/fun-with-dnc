...
class DNCMemory(nn.Module):
    ...
    def forward(self, inputs, prev_state):
        ...
        memory, read_weights, write_weights, prev_linkage, usage = prev_state
        assert list(memory.size()) == [self.batch_size, self.N, self.W]

        # forward pass
        # 1 Update usage using 'free_gate' and previous read & write weights.
        usage = self._usage(write_weights, free_gates, read_weights, usage)

        # 2 Get Write weights
        write_inputs = [alloc_gate, write_str, write_key, write_gate]
        write_weights = self._write_weights(write_inputs, memory, usage)

        sl.log_if('access.calcs.write_weights2', write_weights)
        # 3 Write to memory.
        memory = self._erase_and_write(memory, write_weights, erase_vec, write_vec)
        ...
        read_weights = self._read_weights(read_inputs, memory, read_weights, links)
        #############
        # 6 read memory
...
class DNC(nn.Module):
    ...
    def init_state(self, ):
        """ prev_state: A `DNCState` tuple containing the fields
            ...
            """
        access_output = Variable(torch.zeros(self.batch_size, self.num_reads, self.word_len), requires_grad=True)
        memory_ = [Variable(torch.zeros(self.batch_size, self.mem_size, self.word_len), requires_grad=True), #memory
        ...
        hidden = [Variable(torch.zeros(self.num_layers, self.batch_size, self.hidden_size), requires_grad=True),
                Variable(torch.zeros(self.num_layers, self.batch_size, self.hidden_size), requires_grad=True)]
    ...
    def forward(self, inputs, previous_state):
        ...
        prev_access_output = prev_access_output.view(-1, self.num_reads * self.word_len)
        control_input = torch.cat([inputs, prev_access_output], 1)
        #require_nonleaf_grad(control_input)

        control_output, controller_state = self.controller(control_input, prev_control_state)
