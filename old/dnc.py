...
def repackage(xs):
    ...
    elif type(xs) == nn.Parameter:
        return Variable(xs.data)
    else:
        return tuple(repackage(v) for v in xs)

def Ilog(obj):
    if obj.step % obj.log == 0:
        pass
...
class Usage(nn.Module):
    def __init__(self, batch_size=1, memory_size=100):
        super(Usage, self).__init__()
        self._memory_size = memory_size
        self.usage = Variable(torch.zeros(batch_size, memory_size))
    ...
    def _usage_after_write(self,  write_weights):
        ...
        # print(write_weights)
        #assert prev_usage.size(-1) == self._memory_size
        assert write_weights.size(-1) == self._memory_size
    ...
    def _usage_after_read(self,  free_gate, read_weights):
        ...
        return self.usage * phi

    def forward(self, write_weights, free_gate, read_weights):
        """
            Args:
            write_weights: tensor of shape `[batch_size, num_writes, memory_size]`
            ...
            """
        #write_wghts = write_weights.detach()

        self.usage = self._usage_after_write(write_weights)
        self.usage = self._usage_after_read(free_gate, read_weights)


class Linkage(nn.Module):
    def __init__(self, batch_size=1, memory_size=100, num_writes=1):
...
class DNCMemory(nn.Module):
    def __init__(self, word_size=32, memory_size=100, num_reads=1, write_heads=1, batch_size=32):
        ...
        self.memory = Variable(torch.zeros(batch_size, memory_size, word_size), requires_grad=True) # memory
        self.read_weights = Variable(torch.zeros(batch_size, num_reads, memory_size)) # read_weights
        self.write_weights = Variable(torch.zeros(batch_size, write_heads, memory_size))  # write_weights

        self.links = Variable(torch.zeros(batch_size, write_heads, memory_size, memory_size))
        self.link_weights = Variable(torch.zeros(batch_size, write_heads, memory_size))

        self._linkage = Linkage(memory_size=memory_size, num_writes=write_heads)
        self.usage_mod = Usage(batch_size, memory_size)
    ...
    def get_write_weights(self, inputs, memory, usage):
    ...
    def _erase_and_write(self,  erase_vec, values):
        ...
        expand_address = self.write_weights.unsqueeze(3)
        ...
        self.memory.mul(reset_gate)

        add_matrix = self.write_weights.transpose(1, 2).matmul(values)
    ...
    def forward(self, inputs):
        ...
        self.usage_mod(self.write_weights, free_gates, self.read_weights)

        # 2 Get Write weights
        write_inputs = [alloc_gate, write_str, write_key, write_gate]
        self.write_weights = self.get_write_weights(write_inputs, self.memory, self.usage_mod.usage)

        sl.log_if('access.calcs.write_weights2', self.write_weights)
        # 3 Write to memory.
        self._erase_and_write(erase_vec, write_vec)
        sl.log_if('access.calc.memory_out', self.memory)

        # 4 update linkage
        self.links, self.link_weights = self._linkage(self.write_weights, self.links, self.link_weights)

        # 5 read from memory
        read_inputs = [read_keys, read_str, read_modes]
        self.read_weights = self._read_weights(read_inputs, self.memory, self.read_weights, self.links)

        # 6 read memory
        read_words = self.read_weights.matmul(self.memory)
...
class DNC(nn.Module):
    def __init__(self, batch_size=32, memory_size=100, word_len=6,
        ...
        self.DNCmemory = DNCMemory(word_size=word_len,
                                 memory_size=memory_size,
                                 batch_size=batch_size,
                                 num_reads=num_read_heads)
        self.nn_output = nn.Linear(self.interface_size + word_len + num_read_heads * word_len, self.output_size)
        self.access_out = nn.Parameter(torch.zeros(self.batch_size, self.num_reads, self.word_len))
    ...
    def init_state(self):
        """ prev_state: A `DNCState` tuple containing the fields
            ...
            """
        return [Variable(torch.zeros(self.batch_size, self.num_reads, self.word_len), requires_grad=True),
                 #Variable(torch.zeros(self.batch_size, self.mem_size, self.word_len), requires_grad=True), #memory
                 #Variable(torch.zeros(self.batch_size, self.num_reads, self.mem_size), requires_grad=True), #read_weights
                 #Variable(torch.zeros(self.batch_size, self.num_writes, self.mem_size), requires_grad=True), #write weights
                 # Variable(torch.zeros(self.batch_size, self.num_writes, self.mem_size, self.mem_size), requires_grad=True),  #linkage
                 #Variable(torch.zeros(self.batch_size, self.num_writes, self.mem_size), requires_grad=True), #linkage weight
                 #Variable(torch.zeros(self.batch_size, self.mem_size), requires_grad=True), #usage
                [Variable(torch.randn(self.num_layers, self.batch_size, self.hidden_size), requires_grad=True),
                 Variable(torch.randn(self.num_layers, self.batch_size, self.hidden_size), requires_grad=True)]
                ]
        # hidden =
        #return [access_output, memory_, hidden]

    @property
    def memory(self):
        return self._memory

    def forward(self, inputs, prev_access_output, controller_state):
        """
        ...
        control_input = torch.cat([inputs, prev_access_output.view(-1, self.num_reads * self.word_len)], 1)

        control_output, controller_state = self.controller(control_input, controller_state)
        interface_output = self._interface(control_output)

        access_output = self.DNCmemory(interface_output).view(-1, self.word_len * self.num_reads)

        # access_output = access_output.view(-1, self.word_len * self.num_reads)
        output = torch.cat([control_output, access_output], 1)
        output = self.nn_output(output)

        return output, access_output, controller_state
