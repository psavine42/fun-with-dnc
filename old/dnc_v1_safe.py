...
class DNC(nn.Module):
    def __init__(self, batch_size=32, memory_size=100, word_len=6,
        ...
        self._memory = DNCMemory(word_size=word_len,
                                 ...
                                 batch_size=batch_size,
                                 num_reads=num_read_heads)
        self.nn_output = nn.Linear(self.interface_size + word_len, self.output_size)
    ...
    def forward(self, inputs, previous_state):
        ...
        sizes = prev_access_output.size(1)
        #inputs = inputs.unsqueeze(1)
        ...
        #print("control vec", control_input.size())

        controller_output, controller_state = self._controller(control_input, prev_control_state)
        #print("control out", controller_output.size(), )
        interface_output = self._interface(controller_output)

        #print("interface vec", interface_output.size() )
        ...
        #print("controller_output", controller_output.size())

        output = torch.cat([controller_output, access_output], 1)
        output = self.nn_output(controller_output)
        #print(out_vec.size())
